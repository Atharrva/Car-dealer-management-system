...
<section class="">
		<?php
			...
			$qy = "SELECT * FROM customers WHERE Customer_id='$_GET[id]'";
			$log = $conn->query($qy);
			$num = $log->num_rows;
			$row = $log->fetch_assoc();
			if($num > 0){
		...
		<div class="wrapper">
				...
				<?php
						if(isset($_POST['save'])){
							...
							Gender='$gendera' WHERE Customer_id='$_GET[id]'";
							$result = $conn->query($qry);
							if($result == TRUE){
								echo "<script type = \"text/javascript\">
											alert(\"Successfully Update\");
											window.location = (\"edit_customers.php\")
											</script>";
							} else{
								echo "<script type = \"text/javascript\">
											alert(\"Updation Failed. Try Again\");
											window.location = (\"admin_customer_edit.php\")
											</script>";
							}
						}
